"""Entry point for the OpenBench Publisher MCP server.

This file defines the MCPApp instance that `uvx mcp-agent deploy` expects
to find in ``main.py``. The implementation mirrors ``apps/mcp_agent_main``
so that both local runs and deployed MCP servers share the same tools.
"""
...
async def main() -> None:
    """Optional local test runner for the MCP app."""
    async with app.run() as agent_app:
        print("\n" + "=" * 80)
        print("OpenBench Publisher MCP - Local Test")
        print("=" * 80 + "\n")

        # Example: small test dataset
